...
export async function runFullWorkflow(raw: BasicRequest, store: WorkflowStore<FullSnapshot>, deps: AgentDependencies, options: { signal?: AbortSignal; humanInput?: boolean; resume?: { responseId: string; answers: string[] } } = {}): Promise<FullSnapshot> {
  ...
  let current = options.resume ? parseFullSnapshot(await durable(() => store.load(request.data.runId))) : parseFullSnapshot({ qualityPolicy: 1, version: 2, id: request.data.runId, problemId: request.data.problemId, description: input.data.description,
  ...
  try {
    if (options.resume) {
      ...
      const planned = await runAgent("planner", { problem: understood.output }, deps, options);
      await advance("RESEARCH", { plan: planned.output });
      if (options.humanInput) {
        const questions = [...new Set(planned.output.steps.flatMap((step) => step.userQuestions))];
    ...
    const verified = await runAgent("verifier", { claims: researched.output.claims, sources: researched.sources }, deps, options);
