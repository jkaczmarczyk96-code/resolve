...
export default function PrivacyPage() {
  return (
    <LegalPage title="Privacy Policy" intro="This policy explains what Avenli collects, why it is used, and the choices available to you.">
      ...
      <section><h2>Product analytics</h2><p>When enabled, Avenli records a small set of first-party events such as opening a workspace page or completing onboarding. Paths are normalized so problem identifiers are not stored. These events do not contain problem text, research queries, email content, Google data or authentication URLs, and no third-party analytics script is loaded. You can disable this collection in Settings; existing events remain available in your account export and are deleted with your account.</p></section>
      ...
      <section><h2>Connected Google services</h2><p>If you separately choose to connect Google Calendar or Gmail, Avenli reads upcoming calendar event details or Gmail search results consisting of message metadata and short snippets. Calendar event creation requires an additional permission and your explicit approval of the exact event immediately before it is created. Avenli does not add attendees, send invitations or send email. We store an audit record of proposed and completed actions. Avenli does not sell Google user data or use it for advertising. Access tokens are encrypted, each service can be disabled independently, and you can revoke the complete Google connection from Settings at any time.</p></section>
