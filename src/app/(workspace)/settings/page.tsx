...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ demo?: string; integration?: string }> }) {
  ...
  const client = await createClient(); const profile = await client.from("profiles").select("display_name,avatar_url,timezone,preferred_language,product_analytics_enabled").eq("id", user.id).single();
