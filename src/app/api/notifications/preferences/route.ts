...
export async function POST(request: Request) {
  try {
    ...
    const result = await client.rpc("save_notification_preferences", { p_analysis: input.data.analysis_updates, p_action: input.data.action_required, p_monitoring: input.data.monitoring_updates }).abortSignal(AbortSignal.timeout(10_000));
