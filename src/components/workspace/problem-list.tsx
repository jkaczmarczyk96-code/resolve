...
export function LiveProblemList({ overview = false }: { overview?: boolean }) {
  ...
  const ready = (data ?? []).filter(({ job }) => jobLabel(job) === "Ready to review").length;
  const waiting = (data ?? []).filter(({ job }) => jobLabel(job) === "Needs your input").length;
  const activeCount = (data ?? []).filter(({ job }) => jobActive(job)).length;
