...
function Preferences({ initial }: { initial: z.infer<typeof preferencesSchema> }) {
  ...
  }}>{([{ key: "analysis_updates", label: "Analysis completion and failures" }, { key: "action_required", label: "Requests for your answers" }, { key: "monitoring_updates", label: "Monitoring results and failures" }] as const).map(({ key, label }) => <label key={key} className="flex items-center gap-3 text-sm"><input type="checkbox" checked={value[key]} disabled={pending} onChange={(event) => setValue({ ...value, [key]: event.target.checked })} />{label}</label>)}<Button disabled={pending}>{pending ? "Saving…" : "Save preferences"}</Button>{message && <p role="status" className="text-sm">{message}</p>}</form></Panel>;
}
