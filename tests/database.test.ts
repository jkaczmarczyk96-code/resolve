...
describe("database grants and RLS", () => {
  it("enables RLS and the expected policies on every public table", async () => {
    ...
    for (const row of rows) {
      ...
      expect(row.count, row.relname).toBe(row.relname === "notification_events" ? 0 : ["web_runs", "human_requests", "monitoring_conditions", "notifications", "notification_preferences", "integrations", "integration_events", "external_actions", "external_action_events", "product_events"].includes(row.relname) ? 1 : 4);
    }
  });
