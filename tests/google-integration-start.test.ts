...
vi.mock("@/lib/integrations/google", () => ({
  googleScopesForServices: (services: string[]) => services.map((service) => `${service}.readonly`),
...
it("starts a read-only Google connection from a same-origin navigation", async () => {
  ...
  expect(mock.createState).toHaveBeenCalledWith("owner-id", ["calendar"], ["calendar"]);
});
...
it("preserves an enabled Gmail service while adding Calendar authorization", async () => {
  ...
  expect(mock.createState).toHaveBeenCalledWith("owner-id", ["gmail", "calendar"], ["gmail", "calendar"]);
});
