...
it("exports all owned data but excludes worker credentials and other accounts", async () => {
  const result = await db.query<{ data: { profile: { id: string }; problems: Array<{ id: string }>; web_runs: Array<Record<string, unknown>>; monitoring_conditions: unknown[]; notifications: unknown[] } }>("select public.export_account_data() as data");
  const data = result.rows[0].data; expect(data.profile.id).toBe(owner); expect(data.problems.map((p) => p.id)).toEqual([problem]); expect(data.web_runs[0].secret_token).toBeUndefined(); expect(JSON.stringify(data)).not.toContain("a".repeat(64)); expect(data.monitoring_conditions).toHaveLength(1); expect(data.notifications).toHaveLength(1);
...
it("rejects missing identity and deletes owned workspace data through Auth cascades", async () => {
  ...
  for (const table of ["profiles", "problems", "web_runs", "monitoring_conditions", "notifications", "notification_events"]) {
