...
test("inbox filtering, read acknowledgement and preference persistence", async ({ page, request }) => {
  ...
  await page.route("**/api/notifications", async (route) => {
    ...
    await route.fulfill({ json: { items: [{ id: "11111111-1111-4111-8111-111111111111", problem_id: "22222222-2222-4222-8222-222222222222", kind: "task_due", created_at: new Date().toISOString(), read_at: readAt }], preferences } });
  });
  ...
  await expect(page.getByText("A task is due within 24 hours", { exact: false })).toBeVisible();
  await expect(page.getByRole("link", { name: "Open problem", exact: true })).toHaveAttribute("href", "/problems/22222222-2222-4222-8222-222222222222?view=tasks");
