...
it("renders saved task progress without implying automatic execution", () => {
  const html = renderToStaticMarkup(createElement(TaskList, {
    ...
    saved: [{ id: "de305d54-75b4-431b-adb2-eb6b9e546015", sourceId: "review", status: "completed" as const, completedAt: "2026-09-20T12:00:00.000Z", dueAt: null }],
    refresh: () => undefined,
  }));
  ...
  expect(html).toContain('aria-label="Due date for Review the plan"');
});
