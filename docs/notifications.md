...
Web-run transitions emit `analysis_completed`, `analysis_failed` and `input_required`. Monitoring emits `condition_met` and `monitor_failed`. Labels describe actual saved events; completed analysis is not labelled a solved problem. Existing historical runs are not backfilled.
...
Apply `20260913000000_notifications.sql` before deploying this release. The migration also fixes checkpoint selection after an interrupted clarification continuation: the next claim recovers the latest workflow stage instead of replaying the original question snapshot.
