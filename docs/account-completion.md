...
As of 2026-09-13 both hosted providers are disabled. OAuth integration is implemented but **phase 12 is not fully verified**. Keep `AUTH_GOOGLE_ENABLED` and `AUTH_APPLE_ENABLED` unset until the corresponding Supabase provider is configured and tested.

Google: create a Web application OAuth client in Google Cloud, configure the consent screen, and use `https://omdtlsmhxsudxqvoahfz.supabase.co/auth/v1/callback` as the authorized redirect URI. Enter the client ID/secret in Supabase Authentication → Sign In / Providers → Google. Configure the consent app for the intended audience and add test users if it remains in testing mode.
