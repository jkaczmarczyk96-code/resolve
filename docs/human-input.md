...
The worker claims the job once and receives the protected original checkpoint and saved response. It saves `RESUME`, runs the planner with the original intake plus structured user responses, and continues from the updated `RESEARCH` state. Intake does not run again. Options, critic, decision and tasks receive the same response context; the researcher receives the revised planned question. Existing eight-agent runs without questions remain unchanged.
